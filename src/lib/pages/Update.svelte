...
<div class="flex flex-col gap-5 h-full overflow-y-auto">
  <div>
    <h1 class="text-2xl font-bold text-white">服务端更新</h1>
  ...
  <div class="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-6 flex-shrink-0">
    <div class="flex items-start gap-4">
      ...
      <div class="flex-1">
        <h2 class="text-base font-semibold text-white mb-1">检查更新</h2>
        <p class="text-sm text-[var(--text-secondary)] mb-4">通过 SteamCMD 将 Unturned 服务端更新至最新版本。更新前请先停止服务器。</p>
        <button
          class="px-6 py-2.5 bg-gradient-to-r from-[var(--accent)] to-cyan-600 hover:from-cyan-500 hover:to-[var(--accent)] text-white text-sm font-medium rounded-lg transition-all duration-[var(--transition-normal)] disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer flex items-center gap-2"
          onclick={startUpdate}
          disabled={updating}
        >
          {#if updating}
    ...
    <div class="flex-1 bg-[var(--bg-card)] border border-[var(--border)] rounded-xl flex flex-col min-h-0">
      <div class="flex items-center justify-between px-5 py-3 border-b border-[var(--border)] flex-shrink-0">
        <div class="flex items-center gap-2">
          ...
          <span class="text-sm font-medium text-white">SteamCMD 输出</span>
        </div>
        <button
