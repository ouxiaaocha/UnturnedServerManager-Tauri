...
#[tauri::command]
pub fn should_show_close_dialog(config: State<'_, Arc<Mutex<ConfigService>>>) -> bool {
...
#[tauri::command]
pub fn get_close_preference(config: State<'_, Arc<Mutex<ConfigService>>>) -> AppSettings {
    let cfg = config.lock().unwrap_or_else(|e| e.into_inner());
